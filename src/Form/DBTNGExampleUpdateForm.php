...
/**
 * @file
 * Contains \Drupal\dbtng_example\Form\DBTNGExampleUpdateForm
 */
...
class DBTNGExampleUpdateForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (empty($entries)) {
      $form['no_values'] = array(
        '#value' => $this->t('No entries exist in the table dbtng table.'),
      );
      return $form;
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $count = \Drupal::service('dbtng.storage')->update($entry);
    drupal_set_message($this->t('Updated entry @entry (@count row updated)', array(
      '@count' => $count,
