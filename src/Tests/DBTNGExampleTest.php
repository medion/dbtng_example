...
class DBTNGExampleTest extends WebTestBase {
  ...
  public function testDBTNGExample() {
    // Assert that two entries were inserted at install.
    $result = \Drupal::service('dbtng.storage')->load();
    $this->assertEqual(
  ...
  public function testUI() {
    ...
    $result = \Drupal::service('dbtng.storage')->load(array('surname' => 'Anonymous'));
    $this->drupalGet('/examples/dbtng_example');
    $this->assertEqual(
  ...
  public function testDBTNGExampleStorage() {
    ...
    $dbtng_storage = \Drupal::service('dbtng.storage');
    ...
    $result = $dbtng_storage->load(array('surname' => 'Doe'));
    $this->assertEqual(
      count($result), 2, 'Found two entries in the table with surname = "Doe".'
    );

    // Now find our not-Doe entry.
    $result = $dbtng_storage->load(array('surname' => 'NotDoe'));
    $this->assertEqual(
    ...
    $this->assertNotEqual($dbtng_storage->update((array) $entry), 0, "NotDoe updated to NowDoe.");

    $result = $dbtng_storage->load(array('surname' => 'NowDoe'));
    $this->assertEqual(
      count($result), 1, "Found renamed 'NowDoe' surname");

    // Read only John Doe entry.
    $result = $dbtng_storage->load(array('name' => 'John', 'surname' => 'Doe'));
    $this->assertEqual(
    ...
    $dbtng_storage->update((array) $entry);
